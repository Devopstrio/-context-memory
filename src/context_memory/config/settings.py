...
class Settings(BaseSettings):
    ...
    database_pool_recycle: int = Field(default=3600, ge=60, le=7200, description="Database pool recycle time in seconds")
    ...
    redis_socket_connect_timeout: float = Field(default=5.0, ge=1.0, le=30.0, description="Redis socket connect timeout")
    ...
    jwt_access_token_expire_minutes: int = Field(default=30, ge=5, le=1440, description="JWT access token expiration in minutes")
    ...
    otlp_exporter_endpoint: Optional[str] = Field(default=None, description="OpenTelemetry OTLP exporter endpoint")
