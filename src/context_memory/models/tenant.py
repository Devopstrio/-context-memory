...
class Tenant(Base, UUIDMixin, TimestampMixin):
    ...
    name: Mapped[str] = mapped_column(
        String(255), nullable=False, unique=True, comment="Tenant name"
    )
    ...
    rate_limit: Mapped[int] = mapped_column(
        Integer, default=1000, nullable=False, comment="API rate limit per minute"
    )
    ...
    max_sessions: Mapped[int] = mapped_column(
        Integer, default=1000, nullable=False, comment="Maximum sessions allowed"
    )
